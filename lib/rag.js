 ...
 *
 * Embeddings: @xenova/transformers (all-MiniLM-L6-v2) — runs LOCALLY, no API key needed
 * Generation: Gemini 1.5 Flash — free via API
 * Vector DB:  Qdrant Cloud — free tier
 *
 * Full pipeline:
 *   1. Chunking   — fixed-size 800 chars with 150-char overlap
 *   2. Embedding  — local MiniLM model (384 dims), no external API
 *   3. Storage    — Qdrant Cloud
 *   4. Retrieval  — cosine similarity, top-5
 *   5. Generation — Gemini 1.5 Flash (context-only prompt)
 */
...
// ─── Constants ────────────────────────────────────────────────────────────────
// Ordered list of free models — tried in sequence if one rate-limits
const CHAT_MODELS = [
...
// ─── Lazy-load the local embedding pipeline (downloads model once, then caches) 
let _embedder = null;
async function getEmbedder() {
  if (!_embedder) {
    // Dynamic import so Next.js doesn't try to bundle this at build time
    const { pipeline, env } = await import("@xenova/transformers");
    // Cache model in /tmp for serverless environments
...
// ─── 1. Chunking — Fixed-Size with Overlap ────────────────────────────────────
/**
...
// ─── 2. Embed a single text (local model) ────────────────────────────────────
...
// ─── 7. Generate grounded answer via OpenRouter (with model fallbacks) ────────
